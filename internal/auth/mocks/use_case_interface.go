// Code generated by mockery v2.20.0. DO NOT EDIT.

package mocks

import (
	auth "auth-with-clean-architecture/internal/auth"

	mock "github.com/stretchr/testify/mock"

	user "auth-with-clean-architecture/internal/user"
)

// UseCaseInterface is an autogenerated mock type for the UseCaseInterface type
type UseCaseInterface struct {
	mock.Mock
}

type UseCaseInterface_Expecter struct {
	mock *mock.Mock
}

func (_m *UseCaseInterface) EXPECT() *UseCaseInterface_Expecter {
	return &UseCaseInterface_Expecter{mock: &_m.Mock}
}

// Login provides a mock function with given fields: payload
func (_m *UseCaseInterface) Login(payload *auth.Payload) (*auth.ProfileItemWithToken, error) {
	ret := _m.Called(payload)

	var r0 *auth.ProfileItemWithToken
	var r1 error
	if rf, ok := ret.Get(0).(func(*auth.Payload) (*auth.ProfileItemWithToken, error)); ok {
		return rf(payload)
	}
	if rf, ok := ret.Get(0).(func(*auth.Payload) *auth.ProfileItemWithToken); ok {
		r0 = rf(payload)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*auth.ProfileItemWithToken)
		}
	}

	if rf, ok := ret.Get(1).(func(*auth.Payload) error); ok {
		r1 = rf(payload)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// UseCaseInterface_Login_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Login'
type UseCaseInterface_Login_Call struct {
	*mock.Call
}

// Login is a helper method to define mock.On call
//   - payload *auth.Payload
func (_e *UseCaseInterface_Expecter) Login(payload interface{}) *UseCaseInterface_Login_Call {
	return &UseCaseInterface_Login_Call{Call: _e.mock.On("Login", payload)}
}

func (_c *UseCaseInterface_Login_Call) Run(run func(payload *auth.Payload)) *UseCaseInterface_Login_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(*auth.Payload))
	})
	return _c
}

func (_c *UseCaseInterface_Login_Call) Return(_a0 *auth.ProfileItemWithToken, _a1 error) *UseCaseInterface_Login_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *UseCaseInterface_Login_Call) RunAndReturn(run func(*auth.Payload) (*auth.ProfileItemWithToken, error)) *UseCaseInterface_Login_Call {
	_c.Call.Return(run)
	return _c
}

// ShowProfile provides a mock function with given fields: tokenSigned
func (_m *UseCaseInterface) ShowProfile(tokenSigned string) (*user.User, error) {
	ret := _m.Called(tokenSigned)

	var r0 *user.User
	var r1 error
	if rf, ok := ret.Get(0).(func(string) (*user.User, error)); ok {
		return rf(tokenSigned)
	}
	if rf, ok := ret.Get(0).(func(string) *user.User); ok {
		r0 = rf(tokenSigned)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*user.User)
		}
	}

	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(tokenSigned)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// UseCaseInterface_ShowProfile_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'ShowProfile'
type UseCaseInterface_ShowProfile_Call struct {
	*mock.Call
}

// ShowProfile is a helper method to define mock.On call
//   - tokenSigned string
func (_e *UseCaseInterface_Expecter) ShowProfile(tokenSigned interface{}) *UseCaseInterface_ShowProfile_Call {
	return &UseCaseInterface_ShowProfile_Call{Call: _e.mock.On("ShowProfile", tokenSigned)}
}

func (_c *UseCaseInterface_ShowProfile_Call) Run(run func(tokenSigned string)) *UseCaseInterface_ShowProfile_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(string))
	})
	return _c
}

func (_c *UseCaseInterface_ShowProfile_Call) Return(_a0 *user.User, _a1 error) *UseCaseInterface_ShowProfile_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *UseCaseInterface_ShowProfile_Call) RunAndReturn(run func(string) (*user.User, error)) *UseCaseInterface_ShowProfile_Call {
	_c.Call.Return(run)
	return _c
}

type mockConstructorTestingTNewUseCaseInterface interface {
	mock.TestingT
	Cleanup(func())
}

// NewUseCaseInterface creates a new instance of UseCaseInterface. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewUseCaseInterface(t mockConstructorTestingTNewUseCaseInterface) *UseCaseInterface {
	mock := &UseCaseInterface{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
